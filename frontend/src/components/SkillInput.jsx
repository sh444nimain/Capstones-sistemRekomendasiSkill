...
export default function SkillInput({ session }) {
    ...
    // 1. Ambil ID Pengguna dari sesi Supabase (akan bernilai null jika belum login)
    const userId = session?.user?.id || null;

    // 2. Efek untuk memuat riwayat secara otomatis HANYA JIKA pengguna sudah login
    useEffect(() => {
        const fetchHistory = async () => {
            if (userId) {
                ...
                setHistory([]); // Kosongkan riwayat jika berstatus tamu
            }
        };
        fetchHistory();
    }, [userId]);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            // 3. Kirim data ke backend BESERTA user_id
            const response = await axios.post(
                "http://localhost:5000/analyze",
                {
                    skills: skillText,
                    user_id: userId, // Jika null, backend akan tahu ini adalah tamu
                }
            );
            ...
            // 4. Perbarui daftar riwayat HANYA JIKA pengguna sudah login
            if (userId) {
    ...
    const handleDeleteHistory = async () => {
        try {
            // 5. Sertakan user_id saat memanggil rute hapus
            await axios.delete(
                `http://localhost:5000/history?user_id=${userId}`
            );
            setHistory([]);
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 transition-colors">
            ...
            {/* 6. Render bersyarat: Riwayat HANYA muncul jika ada userId DAN history tidak kosong */}
