...
export default function SkillInput({ session }) {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            if (fullText.trim().length === 0) {
                alert("Waduh, teks di PDF-mu tidak terbaca (kemungkinan berbentuk gambar/scan). Silakan gunakan copy-paste manual atau ganti file PDF yang berbasis teks.");
            } else {
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 transition-colors">
            ...
            <div className="relative">
                ...
                {pdfLoading && (
                    <div className="absolute inset-0 bg-slate-100/10 backdrop-blur-[1px] flex flex-col items-center justify-center rounded-2xl">
                        ...
                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Mengekstrak isi CV...</p>
                    </div>
                )}
            </div>
            ...
            {/* HASIL ANALISIS */}
            {result && (
                <div id="printable-result" className="mt-8 bg-slate-100 dark:bg-slate-800 p-6 rounded-2xl border border-slate-200/20 dark:border-slate-700/20">
